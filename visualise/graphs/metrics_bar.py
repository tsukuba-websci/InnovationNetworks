...
for innovation_type, name in innovation_types.items():
    ...
    file_location = f"../../full_search/results/{innovation_type}/metrics/"

    best_nctf = pd.read_csv(f"{file_location}/best_nctf_{innovation_type}_metrics.csv")
    worst_nctf = pd.read_csv(f"{file_location}/worst_nctf_{innovation_type}_metrics.csv")
    best_ttf = pd.read_csv(f"{file_location}/best_ttf_{innovation_type}_metrics.csv")
    worst_ttf = pd.read_csv(f"{file_location}/worst_ttf_{innovation_type}_metrics.csv")
    ...
    for col, title in zip(cols, titles_readable.values()):
        ...
        ax.set_ylabel(title)
        ax.set_title(f"{title}: {innovation_types[innovation_type]} Innovation")
        ax.set_xticks(index + bar_width / 2)
        ax.set_xticklabels(['NCTF', 'TTF'])
        ax.legend()

        plt.tight_layout()
        plt.savefig(f'../results/metrics_{col}_{innovation_type}.png', dpi=300)
        plt.close(fig)
